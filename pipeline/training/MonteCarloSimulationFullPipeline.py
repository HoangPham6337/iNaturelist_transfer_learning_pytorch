...
from pipeline.training.utility import create_stratified_weighted_sample, get_other_id, save_report
...
class MonteCarloSimulationFullPipeline:
    def __init__(
        self,
        ...
        small_model_name = os.path.basename(small_model_path).replace(".onnx", "")
        big_model_name = os.path.basename(big_model_path).replace(".onnx", "")
        self.model_name = f"{small_model_name}_{big_model_name}"
        ...
        self.small_other_label = get_other_id(self.small_species_labels)
    ...
    def _translate_prediction_to_global_label(
        ...
        if model_type == ModelType.BIG_MODEL:
            ...
            if not global_species_labels:
                print(
                    f"[Warning] Could not map species from big pred {big_species_label} to global label"
                )
            ...
            if not global_species_labels:
                print(
                    f"[Warning] Could not map species from small pred {small_species_label} to global label"
                )
                return self.not_belong_to_global_idx
            return global_species_labels[0]
    ...
    def _infer_with_routing(self, image_path: str, ground_truth: int):
    ...
    def run(
        self, num_runs: int, sample_size: int = 1000, save_path=None, enable_confusion_matrix=False
    ):
        other_preds = 0
        all_true, all_pred = [], []
        for run in range(num_runs):
            ...
            sampled_species = create_stratified_weighted_sample(
                self.global_species_labels, self.global_species_probs, sample_size
            )
            ...
            for species_id in tqdm(
                ...
                result = self._infer_with_routing(image_path, species_id)
        ...
        self.global_species_labels.update({self.not_belong_to_global_idx: "Not in HG dataset"})
        total_support_list.append(num_pred_outside_global)
        unique_ids = list(self.global_species_labels.keys())
